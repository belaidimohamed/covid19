...
class Trainer():
  def __init__(self,config):
    ...
    self.config = config
    self.img_resolution = config['image_resolution']
    self.net = Net(config).to(self.device)
  ...
  def splitData(self):
    ...
    training_data = np.load(self.config['input_data'][0] + 'training_data#'+self.config['transforms']+'.npy',allow_pickle=True)
...
# ------------------------------------------------- Training ----------------------------------------------------
  ...
  def train(self,BATCH_SIZE=64,EPOCHS=80):
  ...
  def test(self):
    ...
    while 1:
        if response == 'n':
            ...
            torch.save(self.net.state_dict(), os.path.join(self.config["output_folder"][0],str(round(correct/total, 3))+"#"+self.config['transforms'] ))
            print('saving done !')
            break
        else :
            pass
